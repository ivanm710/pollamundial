...
// Official FIFA 2026 KO bracket IDs
...
const SF=['M101','M102']
// Feed maps (verified against official FIFA 2026 bracket)
...
// Third-place clusters (verified against official FIFA rules)
...
// Visual bracket layout (FIFA 2026 official top/bottom half structure)
// TOP HALF -> M101 | BOTTOM HALF -> M102
const BW=136,BH=46,BTH=560,BCTW=160
...
const BTW=BXRR+BW
const BLR32=['M74','M77','M73','M75','M83','M84','M81','M82']
const BLR16=['M89','M90','M93','M94']
const BLQF=['M97','M98']
const BLSF=['M101']
const BRSF=['M102']
const BRQF=['M99','M100']
const BRR16=['M91','M92','M95','M96']
const BRR32=['M76','M78','M79','M80','M86','M88','M85','M87']
...
// Build group standings from a picks/results object {matchId:'1'|'X'|'2'}
...
// Build R32 bracket from any picks/results object (used for OFFICIAL bracket from real results)
function buildR32(picks){
  ...
  // Backtracking — guarantees valid assignment for all 495 possible combinations
  ...
  function assignThirds(idx,usedGroups){
    ...
    const candidates=top8.filter(t=>cluster.includes(t.group)&&!usedGroups.has(t.group))
    for(const c of candidates){usedGroups.add(c.group);asgn[slot]=fl(c);if(assignThirds(idx+1,usedGroups))return true;usedGroups.delete(c.group);delete asgn[slot]}
    return false
  }
  ...
  return{
    M73:{h:g('A','p2'),a:g('B','p2')},M74:{h:g('E','p1'),a:asgn.M74||'TBD'},
    M75:{h:g('F','p1'),a:g('C','p2')},M76:{h:g('C','p1'),a:g('F','p2')},
    M77:{h:g('I','p1'),a:asgn.M77||'TBD'},M78:{h:g('E','p2'),a:g('I','p2')},
    M79:{h:g('A','p1'),a:asgn.M79||'TBD'},M80:{h:g('L','p1'),a:asgn.M80||'TBD'},
    M81:{h:g('D','p1'),a:asgn.M81||'TBD'},M82:{h:g('G','p1'),a:asgn.M82||'TBD'},
    M83:{h:g('K','p2'),a:g('L','p2')},M84:{h:g('H','p1'),a:g('J','p2')},
    M85:{h:g('B','p1'),a:asgn.M85||'TBD'},M86:{h:g('J','p1'),a:g('H','p2')},
    M87:{h:g('K','p1'),a:asgn.M87||'TBD'},M88:{h:g('D','p2'),a:g('G','p2')},
  }
}

function nextRound(feedMap,koPicks){
  const out={}
  Object.entries(feedMap).forEach(([id,[m1,m2]])=>{out[id]={h:koPicks[m1]||'TBD',a:koPicks[m2]||'TBD'}})
  return out
}
...
// Apply admin overrides to auto-calculated R32 bracket
function officialR32(results, r32Overrides){
  const b32=buildR32(results||{})
  Object.entries(r32Overrides||{}).forEach(([mid,ov])=>{
    if(b32[mid]&&ov){
      if(ov.h)b32[mid]={...b32[mid],h:ov.h}
      if(ov.a)b32[mid]={...b32[mid],a:ov.a}
    }
  })
  return b32
}

// Hybrid bracket: R32 from official results+overrides, R16+ from USER picks
// This lets each player predict all the way to the final
function userBracket(results,r32Overrides,pid,predictions,lockedPicks){
  const pp=predictions[pid]||{},kp=pp.__k||{},lk=lockedPicks[pid]||{}
  const b32=officialR32(results,r32Overrides)
...
// Official bracket — auto-updates from admin results (used in both Partidos and personal bracket)
function officialBracket(results,r32Overrides,koResults){
  try{
    const b32=officialR32(results,r32Overrides)
...
// ── BRACKET MATCH CARD (module-level, not inside BracketView) ─────────────────
...
function BMatchCard({mid,x,y,bracket,koPicks,koResults,isLocked,readOnly,onKoPick}){
  ...
  function teamRow(team,idx){
    ...
    return <div key={idx} onClick={()=>{if(!locked&&!tbd&&onKoPick)onKoPick(mid,sel?null:team)}}
      style={{height:22,padding:'0 5px',display:'flex',alignItems:'center',background:bg,color:tc,
        fontSize:10,fontWeight:sel||win?700:400,fontFamily:"'Rajdhani',sans-serif",
  ...
  return <div style={{position:'absolute',left:x,top:y,width:BW,border:'1px solid '+borderC,borderRadius:4,overflow:'hidden',background:'#0a1827',boxShadow:'0 2px 6px #00000060',zIndex:2}}>
    {teamRow(m.h,0)}{teamRow(m.a,1)}
  </div>
}

function bktLines(sc,dc,xR,xL,side,kp){
...
function BracketView({bracket,koPicks,koResults,onKoPick,isLocked,readOnly}){
  ...
  const bP={bracket,koPicks,koResults,isLocked,readOnly,onKoPick}
  return <div style={{overflowX:'auto',WebkitOverflowScrolling:'touch',background:'#06101c',borderRadius:8,marginBottom:12,padding:'4px 0'}}>
    <div style={{position:'relative',width:BTW,height:BTH+H+8,flexShrink:0}}>
      {[[BXL0,'16avos'],[BXL1,'8avos'],[BXL2,'4tos'],[BXL3,'Semis'],[BXRS,'Semis'],[BXRQ,'4tos'],[BXRL,'8avos'],[BXRR,'16avos']].map(([x,l])=>(
        <div key={l+x} style={{position:'absolute',top:0,left:x,width:BW,textAlign:'center',fontSize:9,letterSpacing:1.5,color:'#2a4a6a',fontWeight:700,fontFamily:"'Rajdhani',sans-serif",textTransform:'uppercase'}}>{l}</div>
      ))}
      <div style={{position:'absolute',top:0,left:BXCTR,width:BCTW,textAlign:'center',fontSize:9,color:BGLD,fontWeight:700,fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>FINAL {'&'} 3ro</div>
      <svg style={{position:'absolute',top:H,left:0,width:BTW,height:BTH,pointerEvents:'none'}}>
        ...
        <line x1={BXCTR+BCTW/2} y1={finY+BH} x2={BXCTR+BCTW/2} y2={BSF} stroke={BCLR2} strokeWidth="1" strokeDasharray="3,3"/>
        <line x1={BXCTR+BCTW/2} y1={BSF} x2={BXCTR+BCTW/2} y2={thrY} stroke={BCLR2} strokeWidth="1" strokeDasharray="3,3"/>
      </svg>
      {BLR32.map((id,i)=><BMatchCard key={id} mid={id} x={BXL0} y={H+BSC[i] -BH/2} {...bP}/>)}
      {BLR16.map((id,i)=><BMatchCard key={id} mid={id} x={BXL1} y={H+BR16[i]-BH/2} {...bP}/>)}
      {BLQF.map( (id,i)=><BMatchCard key={id} mid={id} x={BXL2} y={H+BQF[i] -BH/2} {...bP}/>)}
      {BLSF.map( (id  )=><BMatchCard key={id} mid={id} x={BXL3} y={H+BSF-BH/2}      {...bP}/>)}
      <div style={{position:'absolute',top:H,left:BXCTR,width:BCTW,height:BTH}}>
        <div style={{position:'absolute',top:finY-14,width:'100%',textAlign:'center',fontSize:9,color:BGLD,fontWeight:700,fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>CAMPEON</div>
        <BMatchCard mid="M104" x={cX} y={finY} {...bP}/>
        <div style={{position:'absolute',top:thrY-14,width:'100%',textAlign:'center',fontSize:9,color:'#c084fc',fontWeight:700,fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>3er PUESTO</div>
        <BMatchCard mid="M103" x={cX} y={thrY} {...bP}/>
      </div>
      {BRSF.map( (id  )=><BMatchCard key={id} mid={id} x={BXRS}  y={H+BSF-BH/2}      {...bP}/>)}
      {BRQF.map( (id,i)=><BMatchCard key={id} mid={id} x={BXRQ}  y={H+BQF[i] -BH/2} {...bP}/>)}
      {BRR16.map((id,i)=><BMatchCard key={id} mid={id} x={BXRL}  y={H+BR16[i]-BH/2} {...bP}/>)}
      {BRR32.map((id,i)=><BMatchCard key={id} mid={id} x={BXRR}  y={H+BSC[i] -BH/2} {...bP}/>)}
    </div>
  </div>
}
  ...
  return <div>
    <div style={{...S.cdk,marginBottom:12}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
        {[['Grupos','3pts','#4d7a9e'],['1/32 - Octavos - Cuartos','5pts','#4a9eff'],['Semis','7pts','#c084fc'],['Final','10pts','#f0c040']].map(([ph,p,c])=>(
...
// ── MATCHES TAB ───────────────────────────────────────────────────────────────
function MatchesTab({results,r32Overrides,koResults,predictions,participants}){
  ...
  // Official bracket auto-updates every time results or koResults change
  const offBracket=useMemo(()=>officialBracket(results,r32Overrides||{},koResults),[results,r32Overrides,koResults])
  return <div>
      ...
      <div style={{...S.cdk,marginBottom:10,border:'1px solid #f0c04033'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:'#4d7a9e'}}>Se actualiza automaticamente con los resultados reales. Verde = equipo que avanza.</div>
          </div>
        </div>
      </div>
      <BracketView bracket={offBracket} koPicks={koResults} koResults={koResults} onKoPick={null} isLocked={()=>true} readOnly={true}/>

      {/* Pick distribution per KO match */}
      <div style={S.crd}>
        <div style={S.ct}>Distribucion de picks - {PL[phase]||phase}</div>
        {(KO_IDS[phase]||[]).map(mid=>{
          const m=offBracket[mid]||{h:'TBD',a:'TBD'}
          if(isTBD(m.h)&&isTBD(m.a))return null
          const koResult=koResults[mid]
          const teams=[m.h,m.a].filter(t=>!isTBD(t))
          // Count only picks for the two actual teams in this match
          const counts={}
          teams.forEach(t=>counts[t]=0)
          let picked=0
          participants.forEach(p=>{
            const pick=(predictions[p.id]?.__k||{})[mid]
            if(!pick)return
            picked++
            // Map pick to current team (handles name changes via overrides)
            if(counts[pick]!==undefined) counts[pick]++
            else {
              // pick doesn't match either current team name — count as "other"
              counts['__other']=(counts['__other']||0)+1
            }
          })
          const total=participants.length
          return <div key={mid} style={{...S.mc,marginBottom:10}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontSize:10,color:'#4d7a9e',fontWeight:700,letterSpacing:1}}>{mid}</span>
              <span style={{fontSize:11,color:'#4d7a9e'}}>{picked}/{total} picks</span>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:10}}>
              <span style={{flex:1,textAlign:'right',fontSize:13,fontWeight:700,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.h}</span>
              <span style={{color:'#4d7a9e',fontSize:9,fontWeight:700,padding:'2px 5px',border:'1px solid #1c3352',borderRadius:4}}>VS</span>
              <span style={{flex:1,fontSize:13,fontWeight:700,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.a}</span>
            </div>
            {teams.filter(t=>t&&!isTBD(t)).map(team=>{
              const cnt=counts[team]||0
              const pct=total>0?Math.round(cnt/total*100):0
              const isWinner=koResult===team
              const barC=isWinner?'#3ddc84':'#4a9eff'
              return <div key={team} style={{marginBottom:8}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                  <span style={{fontSize:11,fontWeight:600,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'65%',color:isWinner?'#3ddc84':'#d8eaf7'}}>
                    {isWinner&&'▶ '}{team}
                  </span>
                  <span style={{fontSize:12,fontWeight:700,color:cnt>0?'#f0c040':'#4d7a9e',flexShrink:0,marginLeft:8}}>
                    {cnt} {total>0&&<span style={{color:'#4d7a9e',fontWeight:400}}>({pct}%)</span>}
                  </span>
                </div>
                <div style={{background:'#1c3352',borderRadius:3,height:6,overflow:'hidden'}}>
                  <div style={{height:'100%',background:barC,width:pct+'%',transition:'width .5s',borderRadius:3}}/>
                </div>
              </div>
            })}
            {picked===0&&<div style={{fontSize:11,color:'#4d7a9e',textAlign:'center',padding:'4px 0'}}>Sin picks aun</div>}
            {(counts['__other']||0)>0&&<div style={{fontSize:10,color:'#4d7a9e',textAlign:'center',padding:'2px 0'}}>
              {counts['__other']} pick(s) para equipos que cambiaron de cruce
            </div>}
          </div>
        })}
        {(KO_IDS[phase]||[]).every(mid=>isTBD((offBracket[mid]||{}).h)&&isTBD((offBracket[mid]||{}).a))&&(
          <div style={{textAlign:'center',color:'#4d7a9e',fontSize:13,padding:'20px 0'}}>
            Esta ronda aun no tiene equipos confirmados.
          </div>
        )}
      </div>
    </div>}
...
function PicksTab({predictions,participants,lockedPicks,onPick,onLockSection,onOpenBracket}){
  ...
  return <div>
    ...
    <button style={{...S.blk,width:'100%',marginBottom:12,fontSize:14,background:'#1a2a0a',color:'#f0c040',border:'1px solid #f0c04066'}} onClick={()=>onOpenBracket(pid)}>
      {'\uD83C\uDFC6'} Ver Bracket Eliminatorio Oficial
    </button>
...
// ── BRACKET TAB ───────────────────────────────────────────────────────────────
// Bracket is now driven by OFFICIAL RESULTS, not personal group predictions.
// Each player picks winners round by round on the real tournament bracket.
function BracketTab({pid,participants,predictions,lockedPicks,results,r32Overrides,koResults,onKoPick,onLockKoSection,onBack}){
  const[lockModal,setLockModal]=useState(null)
  const[viewMode,setViewMode]=useState('bracket')
  const selP=participants.find(p=>p.id===pid)
  const pp=predictions[pid]||{},kp=pp.__k||{},lk=lockedPicks[pid]||{}
  // Hybrid bracket: R32 from official results, R16+ from user's own picks
  const bracket=useMemo(()=>userBracket(results||{},r32Overrides||{},pid,predictions,lockedPicks),[results,r32Overrides,pid,predictions,lockedPicks])

  // A round is available when:
  // 1. Previous round is locked by this user (picks are final)
  // 2. The official bracket has real teams for this round (not all TBD)
  function roundHasTeams(ids){return ids.some(id=>bracket[id]&&!isTBD(bracket[id].h)&&!isTBD(bracket[id].a))}
  ...
  return <div>
    ...
    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:10,color:'#4d7a9e'}}>Bracket Oficial - Elige ganadores</div>
      </div>
...
// ── ADMIN TAB ─────────────────────────────────────────────────────────────────
function AdminTab({participants,results,koResults,r32Overrides,predictions,lockedPicks,adminPin,updParticipants,updResults,updKoResults,updR32Overrides,updLockedPicks,updAdminPin,onReset,adminUnlocked,onNeedPin}){
  const[sec,setSec]=useState('participants')
  const[newName,setNewName]=useState('')
  const[resGroup,setResGroup]=useState('A')
  const[koPh,setKoPh]=useState('Ronda de 32')
  const[koMode,setKoMode]=useState('resultados') // 'resultados' | 'corregir'
  ...
  return <div>
    ...
    {sec==='ko'&&adminUnlocked&&<>
      <div style={{...S.cdk,border:'1px solid #3ddc8433'}}>
        <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.7}}><b style={{color:'#3ddc84'}}>{'\u2705'} Admin activo</b><br/>Registra el equipo que avanza en cada partido. El bracket de todos se actualiza automaticamente.</div>
      </div>
      <div style={{display:'flex',gap:6,overflowX:'auto',paddingBottom:8,marginBottom:12,scrollbarWidth:'none'}}>
        {KO_PH.map(ph=><button key={ph} onClick={()=>{setKoPh(ph);setKoMode('resultados')}} style={{whiteSpace:'nowrap',padding:'5px 11px',borderRadius:20,border:'1px solid',borderColor:ph===koPh?'#3ddc84':'#1c3352',background:ph===koPh?'#1a3a1a':'transparent',color:ph===koPh?'#3ddc84':'#4d7a9e',fontSize:11,fontWeight:700,fontFamily:"'Rajdhani',sans-serif",cursor:'pointer'}}>{PL[ph]||ph}</button>)}
      </div>

      {/* Mode toggle: only shown for R32 */}
      {koPh==='Ronda de 32'&&(
        <div style={{display:'flex',gap:6,marginBottom:12}}>
          {[['resultados','✅ Resultados'],['corregir','✏️ Corregir Cruces']].map(([m,lbl])=>(
            <button key={m} onClick={()=>setKoMode(m)} style={{flex:1,padding:'8px 4px',borderRadius:7,border:'1px solid',borderColor:koMode===m?'#f0c040':'#1c3352',background:koMode===m?'#f0c04015':'transparent',color:koMode===m?'#f0c040':'#4d7a9e',fontFamily:"'Rajdhani',sans-serif",fontSize:12,fontWeight:700,cursor:'pointer'}}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      {/* Corregir cruces de 16avos */}
      {koPh==='Ronda de 32'&&koMode==='corregir'&&(()=>{
        const autoR32=buildR32(results)
        return <div>
          <div style={{...S.cdk,border:'1px solid #f0c04033',marginBottom:12}}>
            <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.7}}>
              {'⚠️'} <b style={{color:'#f0c040'}}>Corregir emparejamientos de 16avos.</b><br/>
              Si la app no calculo correctamente algun cruce (especialmente con los mejores terceros),
              edita los equipos aqui. Los cambios se aplican a todos los brackets de inmediato.
              Deja vacio para restaurar el calculo automatico.
            </div>
          </div>
          {R32.map(mid=>{
            const auto=autoR32[mid]||{h:'TBD',a:'TBD'}
            const ov=r32Overrides[mid]||{}
            const hasOv=!!(ov.h||ov.a)
            const curH=ov.h!==undefined?ov.h:auto.h
            const curA=ov.a!==undefined?ov.a:auto.a
            return <div key={mid} style={{...S.mc,marginBottom:8,borderColor:hasOv?'#f0c04066':'#1c3352'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                <span style={{fontSize:11,color:hasOv?'#f0c040':'#4d7a9e',fontWeight:700}}>{mid}{hasOv&&' ✏️ corregido'}</span>
                {hasOv&&<button onClick={()=>{const nv={...r32Overrides};delete nv[mid];updR32Overrides(nv)}} style={{fontSize:10,padding:'2px 8px',borderRadius:4,border:'1px solid #4d7a9e',background:'transparent',color:'#4d7a9e',cursor:'pointer',fontFamily:"'Rajdhani',sans-serif"}}>Restaurar auto</button>}
              </div>
              <div style={{display:'flex',gap:6}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:'#4d7a9e',marginBottom:3,fontWeight:700}}>LOCAL (auto: {auto.h})</div>
                  <input style={{...S.inp,fontSize:12}} placeholder={auto.h} value={curH===auto.h&&!ov.h?'':curH}
                    onChange={e=>{const nv={...r32Overrides,[mid]:{...(r32Overrides[mid]||{}),h:e.target.value||undefined}};if(!nv[mid].h&&!nv[mid].a)delete nv[mid];updR32Overrides(nv)}}/>
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:'#4d7a9e',marginBottom:3,fontWeight:700}}>VISITANTE (auto: {auto.a})</div>
                  <input style={{...S.inp,fontSize:12}} placeholder={auto.a} value={curA===auto.a&&!ov.a?'':curA}
                    onChange={e=>{const nv={...r32Overrides,[mid]:{...(r32Overrides[mid]||{}),a:e.target.value||undefined}};if(!nv[mid].h&&!nv[mid].a)delete nv[mid];updR32Overrides(nv)}}/>
                </div>
              </div>
            </div>
          })}
      ...
      {/* Resultados de KO */}
      {(koPh!=='Ronda de 32'||koMode==='resultados')&&koIds.map(mid=>{
    ...
    {sec==='settings'&&adminUnlocked&&<>
      <div style={S.crd}>
        ...
        <div style={{marginBottom:8}}><div style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,marginBottom:4,fontWeight:700}}>PIN ACTUAL</div><input style={S.inp} type="password" inputMode="numeric" placeholder="PIN actual" value={oldPin} onChange={e=>setOldPin(e.target.value)}/></div>
        <div style={{marginBottom:8}}><div style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,marginBottom:4,fontWeight:700}}>NUEVO PIN (min. 4)</div><input style={S.inp} type="password" inputMode="numeric" placeholder="Nuevo PIN" value={newPin} onChange={e=>setNewPin(e.target.value)}/></div>
        <div style={{marginBottom:12}}><div style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,marginBottom:4,fontWeight:700}}>CONFIRMAR PIN</div><input style={S.inp} type="password" inputMode="numeric" placeholder="Repite" value={cPin} onChange={e=>setCPin(e.target.value)}/></div>
...
export default function App(){
  ...
  const[r32Overrides,setR32Ov]=useState({})
  ...
  useEffect(()=>{
    ...
    try{
      const unsub=onValue(ref(db,DB_PATH),snap=>{
        try{const d=snap.val();if(d){if(d.participants)setP(toArr(d.participants));if(d.results)setR(d.results||{});if(d.koResults)setKoR(d.koResults||{});if(d.r32Overrides)setR32Ov(d.r32Overrides||{});if(d.predictions)setPreds(d.predictions||{});if(d.lockedPicks)setLocked(d.lockedPicks||{});if(d.adminPin)setAdminPin(d.adminPin)}}catch(e){}
        setLoading(false)
  ...
  function bun(o={}){return{participants,results,koResults,predictions,lockedPicks,adminPin,r32Overrides,...o}}
  ...
  function updR32Ov(v){setR32Ov(v);save(bun({r32Overrides:v}))}
  ...
  // No more bracket generation — just navigate to bracket tab
  function openBracket(pid){setBracketPid(pid);setTab('bracket')}
  function handleNeedPin(cb){setPendingCb(()=>cb);setShowPin(true)}
  function handleReset(){
    setP([]);setR({});setKoR({});setR32Ov({});setPreds({});setLocked({});setAdminPin(DEFAULT_PIN)
    save({participants:[],results:{},koResults:{},predictions:{},lockedPicks:{},adminPin:DEFAULT_PIN,r32Overrides:{}})
  }
  ...
  if(loading)return(<div style={{...S.app,alignItems:'center',justifyContent:'center',fontSize:18,color:'#f0c040'}}>{'\u26BD'} Cargando...</div>)
  ...
  return(
    <div style={S.app}>
      ...
      <div style={S.con}>
        ...
        {tab==='matches'&&<MatchesTab results={results} r32Overrides={r32Overrides} koResults={koResults} predictions={predictions} participants={participants}/>}
        ...
        {tab==='bracket'&&bracketPid&&<BracketTab pid={bracketPid} participants={participants} predictions={predictions} lockedPicks={lockedPicks} results={results} r32Overrides={r32Overrides} koResults={koResults} onKoPick={handleKoPick} onLockKoSection={handleLock} onBack={()=>setTab('picks')}/>}
        {tab==='admin'  &&<AdminTab participants={participants} results={results} koResults={koResults} r32Overrides={r32Overrides} predictions={predictions} lockedPicks={lockedPicks} adminPin={adminPin} updParticipants={updP} updResults={updR} updKoResults={updKoR} updR32Overrides={updR32Ov} updLockedPicks={updLk} updAdminPin={updPin} onReset={handleReset} adminUnlocked={adminUnlocked} onNeedPin={handleNeedPin}/>}
      </div>
      <div style={S.tab}>
        {tabs.map(t=>(
