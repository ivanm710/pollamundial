...
function buildR32(picks){
  ...
  const slots=Object.entries(TC).map(([slot,cl])=>({slot,cl,cnt:top8.filter(t=>cl.includes(t.group)).length})).sort((a,b)=>a.cnt-b.cnt)
  const asgn={}
  slots.forEach(({slot,cl})=>{const c=top8.find(t=>cl.includes(t.group)&&!used.has(t.group));if(c){asgn[slot]=fl(c);used.add(c.group)}else asgn[slot]='TBD'})
...
// ── BRACKET VIEW ──────────────────────────────────────────────────────────────
const BW=136,BH=46,BSH=70,BTH=560,BCTW=160,BCOLW=160
const BSC=[35,105,175,245,315,385,455,525]
const BR16=[70,210,350,490]
const BQF=[140,420]
const BSF=280
const BXL0=0,BXL1=160,BXL2=320,BXL3=480
const BXCTR=640,BXRS=BXCTR+BCTW,BXRQ=BXRS+160,BXRL=BXRS+320,BXRR=BXRS+480
const BTW=BXRR+BW
...
function BracketView({bracket,koPicks,koResults,onKoPick,isLocked,readOnly}){
  ...
  function BCard({mid,x,y}){
    ...
    const phase=getPhase(mid),locked=(isLocked?.(phase))||readOnly||false
    function Row({team,idx}){
      ...
      return <div onClick={()=>{if(!locked&&!tbd&&onKoPick)onKoPick(mid,sel?null:team)}}
        style={{height:22,padding:'0 5px',display:'flex',alignItems:'center',background:bg,color:tc,fontSize:10,fontWeight:sel||win?700:400,fontFamily:"'Rajdhani',sans-serif",overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',cursor:!locked&&!tbd?'pointer':'default',userSelect:'none',borderBottom:idx===0?'1px solid #1c3352':'none'}}>
        {ok&&'\u2713 '}{wrong&&'\u2717 '}{win&&!ok?'\u25B6 ':''}{team||'TBD'}
      </div>
    }
    return <div style={{position:'absolute',left:x,top:y,width:BW,border:'1px solid '+(picked?GLD+'77':win?'#3ddc8477':LC),borderRadius:4,overflow:'hidden',background:'#0a1827',boxShadow:'0 2px 6px #00000060',zIndex:2}}>
      <Row team={m.h} idx={0}/><Row team={m.a} idx={1}/>
    </div>
  }
  const CL='#1e3a5a',CL2='#f0c04055'
  function LC2({sc,dc,xr,xl,k}){
    const mx=(xr+xl)/2
    return <>{dc.map((dy,i)=>{const y1=sc[i*2],y2=sc[i*2+1];return <g key={k+i}>
      <line x1={xr} y1={y1} x2={mx} y2={y1} stroke={CL} strokeWidth="1"/>
      <line x1={xr} y1={y2} x2={mx} y2={y2} stroke={CL} strokeWidth="1"/>
      <line x1={mx} y1={y1} x2={mx} y2={y2} stroke={CL} strokeWidth="1"/>
      <line x1={mx} y1={dy} x2={xl} y2={dy} stroke={CL} strokeWidth="1"/>
    </g>})}</>
  }
  function RC({sc,dc,xl,xr,k}){
    const mx=(xl+xr)/2
    return <>{dc.map((dy,i)=>{const y1=sc[i*2],y2=sc[i*2+1];return <g key={k+i}>
      <line x1={xl} y1={y1} x2={mx} y2={y1} stroke={CL} strokeWidth="1"/>
      <line x1={xl} y1={y2} x2={mx} y2={y2} stroke={CL} strokeWidth="1"/>
      <line x1={mx} y1={y1} x2={mx} y2={y2} stroke={CL} strokeWidth="1"/>
      <line x1={mx} y1={dy} x2={xr} y2={dy} stroke={CL} strokeWidth="1"/>
    </g>})}</>
  }
  const H=22,finY=BSF-BH-12,thrY=BSF+12,cX=BXCTR+(BCTW-BW)/2
  return <div style={{overflowX:'auto',WebkitOverflowScrolling:'touch',background:'#06101c',borderRadius:8,marginBottom:12,padding:'4px 0'}}>
    <div style={{position:'relative',width:BTW,height:BTH+H+8,flexShrink:0}}>
      ...
      <div style={{position:'absolute',top:0,left:BXCTR,width:BCTW,textAlign:'center',fontSize:9,letterSpacing:1.5,color:GLD,fontWeight:700,fontFamily:"'Rajdhani',sans-serif"}}>
        FINAL {'&'} 3er PUESTO
      </div>
      <svg style={{position:'absolute',top:H,left:0,width:BTW,height:BTH,pointerEvents:'none'}}>
        <LC2 sc={BSC}  dc={BR16} xr={BXL0+BW} xl={BXL1}      k="a"/>
        <LC2 sc={BR16} dc={BQF}  xr={BXL1+BW} xl={BXL2}      k="b"/>
        <LC2 sc={BQF}  dc={[BSF]}xr={BXL2+BW} xl={BXL3}      k="c"/>
        <line x1={BXL3+BW} y1={BSF} x2={BXCTR} y2={BSF} stroke={CL2} strokeWidth="1.5"/>
        <RC sc={BSC}  dc={BR16} xl={BXRR}    xr={BXRL+BW}  k="d"/>
        <RC sc={BR16} dc={BQF}  xl={BXRL}    xr={BXRQ+BW}  k="e"/>
        <RC sc={BQF}  dc={[BSF]}xl={BXRQ}    xr={BXRS+BW}  k="f"/>
        <line x1={BXRS} y1={BSF} x2={BXCTR+BCTW} y2={BSF} stroke={CL2} strokeWidth="1.5"/>
        <line x1={BXCTR+BCTW/2} y1={finY+BH} x2={BXCTR+BCTW/2} y2={BSF} stroke={CL2} strokeWidth="1" strokeDasharray="3,3"/>
        <line x1={BXCTR+BCTW/2} y1={BSF} x2={BXCTR+BCTW/2} y2={thrY} stroke={CL2} strokeWidth="1" strokeDasharray="3,3"/>
      </svg>
      {BLR32.map((id,i)=><BCard key={id} mid={id} x={BXL0} y={H+BSC[i] -BH/2}/>)}
      {BLR16.map((id,i)=><BCard key={id} mid={id} x={BXL1} y={H+BR16[i]-BH/2}/>)}
      {BLQF.map( (id,i)=><BCard key={id} mid={id} x={BXL2} y={H+BQF[i] -BH/2}/>)}
      {BLSF.map( (id  )=><BCard key={id} mid={id} x={BXL3} y={H+BSF-BH/2}/>)}
      <div style={{position:'absolute',top:H,left:BXCTR,width:BCTW,height:BTH}}>
        ...
        <BCard mid="M104" x={cX-BXCTR} y={finY}/>
        ...
        <BCard mid="M103" x={cX-BXCTR} y={thrY}/>
      </div>
      {BRSF.map( (id  )=><BCard key={id} mid={id} x={BXRS}  y={H+BSF-BH/2}/>)}
      {BRQF.map( (id,i)=><BCard key={id} mid={id} x={BXRQ}  y={H+BQF[i] -BH/2}/>)}
      {BRR16.map((id,i)=><BCard key={id} mid={id} x={BXRL}  y={H+BR16[i]-BH/2}/>)}
      {BRR32.map((id,i)=><BCard key={id} mid={id} x={BXRR}  y={H+BSC[i] -BH/2}/>)}
    </div>
  </div>
}
...
function MatchesTab({results,koResults,predictions,participants}){
  ...
  return <div>
    ...
    {!isGrp&&<div>
      ...
      <BracketView
        bracket={officialBracket(results,koResults)}
        koPicks={koResults}
        koResults={koResults}
        onKoPick={null}
        isLocked={()=>true}
...
export default function App(){
  ...
  function openBracket(pid){
    ...
    if(!pp.__b||!Object.keys(pp.__b).length){
      const b=buildR32(pp);const newPp={...pp,__b:b};updPr({...predictions,[pid]:newPp})
    }
  ...
  return(
    <div style={S.app}>
      ...
      <div style={S.con}>
        ...
        {tab==='picks'  &&tab!=='bracket'&&<PicksTab predictions={predictions} participants={participants} lockedPicks={lockedPicks} onPick={handlePick} onLockSection={handleLock} onOpenBracket={openBracket}/>}
