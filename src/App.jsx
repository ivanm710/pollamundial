...
function tl(flag,name){ return flag+' '+name }
...
// Official FIFA 2026 Round of 32 bracket structure
const R32_MATCHES = [
  {id:'M73',label:'M73',phase:'Ronda de 32'},
  {id:'M74',label:'M74',phase:'Ronda de 32'},
  {id:'M75',label:'M75',phase:'Ronda de 32'},
  {id:'M76',label:'M76',phase:'Ronda de 32'},
  {id:'M77',label:'M77',phase:'Ronda de 32'},
  {id:'M78',label:'M78',phase:'Ronda de 32'},
  {id:'M79',label:'M79',phase:'Ronda de 32'},
  {id:'M80',label:'M80',phase:'Ronda de 32'},
  {id:'M81',label:'M81',phase:'Ronda de 32'},
  {id:'M82',label:'M82',phase:'Ronda de 32'},
  {id:'M83',label:'M83',phase:'Ronda de 32'},
  {id:'M84',label:'M84',phase:'Ronda de 32'},
  {id:'M85',label:'M85',phase:'Ronda de 32'},
  {id:'M86',label:'M86',phase:'Ronda de 32'},
  {id:'M87',label:'M87',phase:'Ronda de 32'},
  {id:'M88',label:'M88',phase:'Ronda de 32'},
]
...
const R16_IDS   = Object.keys(R16_FEEDS)
const QF_IDS    = Object.keys(QF_FEEDS)
const SF_IDS    = Object.keys(SF_FEEDS)
const KO_ROUND_PHASES = {
  'Ronda de 32':   R32_MATCHES.map(m=>m.id),
  'Octavos de Final': R16_IDS,
  'Cuartos de Final': QF_IDS,
  'Semifinales':   SF_IDS,
  'Tercer Puesto': ['M103'],
  'Gran Final':    ['M104'],
}
// 3rd-place clusters (Match -> which groups' 3rd can play there)
...
function calcGroupStandings(pid, predictions){
  ...
  GROUPS.forEach(g=>{
    const pts=[0,0,0,0]
    const wins=[0,0,0,0]   // victorias (para desempatar mejores terceros)
    const draws=[0,0,0,0]  // empates
    const picks={} // key: 'i-j' -> '1'|'X'|'2'
    const pairs=[[0,1],[2,3],[0,2],[1,3],[0,3],[1,2]]

    pairs.forEach(([i,j])=>{
      const pick=pp['G'+g+i+j]||''
      picks[i+'-'+j]=pick
      if(pick==='1'){ pts[i]+=3; wins[i]++ }
      else if(pick==='X'){ pts[i]+=1; pts[j]+=1; draws[i]++; draws[j]++ }
      else if(pick==='2'){ pts[j]+=3; wins[j]++ }
    })

    // Resultado directo entre dos equipos empatados en puntos → [ptsA, ptsB]
    function h2h(a,b){
      const lo=Math.min(a,b),hi=Math.max(a,b)
      const r=picks[lo+'-'+hi]
      if(r==='1') return a===lo?[3,0]:[0,3]
      if(r==='2') return a===hi?[3,0]:[0,3]
      if(r==='X') return [1,1]
      return [0,0]
    }

    // Criterios de desempate DENTRO del grupo:
    // 1) Puntos totales
    // 2) Resultado directo (head-to-head)
    // 3) Victorias totales
    // 4) Posicion original del sorteo
    const rank=[0,1,2,3].sort((a,b)=>{
      if(pts[b]!==pts[a]) return pts[b]-pts[a]
      const [ha]=h2h(a,b);const [hb]=h2h(b,a)
    ...
    const t=TEAMS[g]
    standings[g]={
      p1:{name:t[rank[0]][0],flag:t[rank[0]][1],pts:pts[rank[0]],wins:wins[rank[0]],draws:draws[rank[0]],seed:rank[0]},
      p2:{name:t[rank[1]][0],flag:t[rank[1]][1],pts:pts[rank[1]],wins:wins[rank[1]],draws:draws[rank[1]],seed:rank[1]},
      p3:{name:t[rank[2]][0],flag:t[rank[2]][1],pts:pts[rank[2]],wins:wins[rank[2]],draws:draws[rank[2]],seed:rank[2]},
      p4:{name:t[rank[3]][0],flag:t[rank[3]][1],pts:pts[rank[3]],wins:wins[rank[3]],draws:draws[rank[3]],seed:rank[3]},
    }
  })
  return standings
}
...
function generateR32(pid, predictions){
  ...
  // Rank 12 third-place teams to find the 8 best qualifiers.
  // Criteria (without goal tracking): pts → wins → draws → group letter
  const thirds = GROUPS.map(g=>({group:g,...s[g].p3}))
    .sort((a,b)=>
      b.pts-a.pts ||         // 1. Puntos
      b.wins-a.wins ||       // 2. Victorias (proxy de diferencia de gol)
      b.draws-a.draws ||     // 3. Empates
      a.group.localeCompare(b.group) // 4. Letra del grupo (desempate final)
    )
  const top8   = thirds.slice(0,8)
  const used   = new Set()
  // Sort clusters by fewest available candidates (most constrained first)
  const slotOrder = Object.entries(THIRD_CLUSTERS).map(([slot,cl])=>({
    slot,cl,cnt:top8.filter(t=>cl.includes(t.group)).length
  })).sort((a,b)=>a.cnt-b.cnt)
  const assigned={}
  slotOrder.forEach(({slot,cl})=>{
    const c=top8.find(t=>cl.includes(t.group)&&!used.has(t.group))
    if(c){assigned[slot]=fl(c);used.add(c.group)}
    else assigned[slot]='TBD'
  })
  const g=(group,pos)=>fl(s[group][pos])
  return {
    M73:{h:g('A','p2'),a:g('B','p2')},
    M74:{h:g('E','p1'),a:assigned.M74||'TBD'},
    M75:{h:g('F','p1'),a:g('C','p2')},
    M76:{h:g('C','p1'),a:g('F','p2')},
    M77:{h:g('I','p1'),a:assigned.M77||'TBD'},
    M78:{h:g('E','p2'),a:g('I','p2')},
    M79:{h:g('A','p1'),a:assigned.M79||'TBD'},
    M80:{h:g('L','p1'),a:assigned.M80||'TBD'},
    M81:{h:g('D','p1'),a:assigned.M81||'TBD'},
    M82:{h:g('G','p1'),a:assigned.M82||'TBD'},
    M83:{h:g('K','p2'),a:g('L','p2')},
    M84:{h:g('H','p1'),a:g('J','p2')},
    M85:{h:g('B','p1'),a:assigned.M85||'TBD'},
    M86:{h:g('J','p1'),a:g('H','p2')},
    M87:{h:g('K','p1'),a:assigned.M87||'TBD'},
    M88:{h:g('D','p2'),a:g('G','p2')},
  }
}

function generateNextRound(feedMap, prevKo){
  const out={}
  Object.entries(feedMap).forEach(([mId,[m1,m2]])=>{
    out[mId]={h:prevKo[m1]||'TBD',a:prevKo[m2]||'TBD'}
  })
  return out
}
...
function generateFinal(sfKo, sfBracket){
  ...
  const b1=sfBracket['M101']||{h:'TBD',a:'TBD'}
  const b2=sfBracket['M102']||{h:'TBD',a:'TBD'}
  const l1 = w1==='TBD'?'TBD':(w1===b1.h?b1.a:b1.h)
  const l2 = w2==='TBD'?'TBD':(w2===b2.h?b2.a:b2.h)
  return {M103:{h:l1,a:l2},M104:{h:w1,a:w2}}
}

// Full bracket for a user (all rounds merged)
function getFullBracket(pid, predictions, lockedPicks){
  const pp   = predictions[pid]||{}
  const lk   = lockedPicks[pid]||{}
  const koPicks = pp.__k||{}
  const b32  = pp.__b||{}
  if(!Object.keys(b32).length) return null

  const b16  = lk['Ronda de 32']   ? generateNextRound(R16_FEEDS,koPicks) : {}
  const bqf  = lk['Octavos de Final'] ? generateNextRound(QF_FEEDS,koPicks) : {}
  const bsf  = lk['Cuartos de Final'] ? generateNextRound(SF_FEEDS,koPicks) : {}
  const bfin = lk['Semifinales']    ? generateFinal(koPicks,bsf) : {}

  return {...b32,...b16,...bqf,...bsf,...bfin}
}

// ── SCORING ───────────────────────────────────────────────────────────────────
function calcBoard(participants, predictions, results, koResults, lockedPicks){
  return [...participants].map(p=>{
    ...
    GROUP_MATCHES.forEach(m=>{
      if(pp[m.id]&&results[m.id]){played++;if(pp[m.id]===results[m.id]){pts+=3;correct++}} // 3pts por acertar 1, X o 2
    })
    // Knockout
    Object.entries(koResults||{}).forEach(([mid,winner])=>{
      const userPick=kp[mid]
      if(userPick){
        played++
        if(userPick===winner){
          const phase=getKOPhase(mid)
          pts+=PHASE_POINTS[phase]||5
          correct++
        }
      }
    })
...
function getKOPhase(mid){
  if(R32_MATCHES.find(m=>m.id===mid)) return 'Ronda de 32'
...
const S={
  ...
  btnGreen:{background:'#1a3a1a',color:'#3ddc84',border:'1px solid #3ddc8466',borderRadius:6,padding:'10px 18px',fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:13,cursor:'pointer'},
...
function BoardTab({board,results,koResults}){
  const grpPlayed=Object.keys(results).length
  const koPlayed=Object.keys(koResults).length
  const total=GROUP_MATCHES.length+16+8+4+2+1+1
  const pct=total>0?Math.round((grpPlayed+koPlayed)/total*100):0
      ...
      <div style={S.cardD}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:'#4d7a9e',marginTop:5}}>{grpPlayed+koPlayed} de {total} partidos con resultado</div>
          </div>
...
function MatchesTab({results,koResults,predictions,participants}){
  ...
  const matches = isGrp ? GROUP_MATCHES.filter(m=>m.group===group) : []
  const koPhaseIds = !isGrp ? (KO_ROUND_PHASES[phase]||[]) : []
  ...
  return(
    <div>
      ...
      {isGrp&&matches.map(m=>{
        ...
        const tot=participants.length;const pts=3;const ptC='#4d7a9e'
        return(
          <div key={m.id} style={S.mc}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,fontWeight:700}}>JORNADA {m.matchday}<span style={{color:ptC,fontWeight:800,marginLeft:6}}>3pts</span></span>
            ...
            {tot>0&&<div style={{display:'flex',gap:4,marginTop:10}}>
              {['1','X','2'].map(opt=>{const c=counts[opt];const pc=tot>0?Math.round(c/tot*100):0;return(
      ...
      {!isGrp&&(
        <div style={S.cardD}>
          <div style={{fontSize:13,color:'#4d7a9e',lineHeight:1.8,textAlign:'center',padding:'20px 0'}}>
            Los partidos de eliminatorias se ven en la seccion<br/><b style={{color:'#d8eaf7'}}>{'\uD83C\uDFAF'} Bracket</b> dentro de Pronosticos.<br/>Ahi cada participante tiene su bracket personalizado.
          </div>
        </div>
      )}
    </div>
...
function PicksTab({predictions,participants,lockedPicks,onPick,onLockSection,onOpenBracket}){
  ...
  return(
    <div>
      ...
      <div style={S.card}>
        ...
        {selP&&<>
          <div style={{display:'flex',alignItems:'center',gap:8,marginTop:10}}>
            ...
            <div style={{flex:1,fontSize:12,color:'#4d7a9e'}}>{predCount}/{GROUP_MATCHES.length} de fase de grupos</div>
      ...
      {allGroupsLocked&&(
        <button style={{...S.btnGreen,width:'100%',marginBottom:12,fontSize:15}} onClick={()=>onOpenBracket(pid)}>
      ...
      <div style={{display:'flex',gap:6,overflowX:'auto',paddingBottom:8,marginBottom:10,scrollbarWidth:'none'}}>
        {GROUPS.map(g=>{
          const locked=!!(pid&&lockedPicks[pid]&&lockedPicks[pid]['GRP-'+g])
          return(
            <button key={g} onClick={()=>setGroup(g)} style={{width:36,height:36,borderRadius:6,border:'1px solid',flexShrink:0,borderColor:g===group?'#4a9eff':'#1c3352',background:g===group?'#1e3a5f':'transparent',color:g===group?'#4a9eff':'#4d7a9e',fontSize:11,fontWeight:700,fontFamily:"'Rajdhani',sans-serif",cursor:'pointer',position:'relative'}}>
              {g}{locked&&<span style={{position:'absolute',top:-4,right:-4,fontSize:8}}>{'\uD83D\uDD12'}</span>}
            </button>
          )
        })}
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10,padding:'8px 12px',background:'#0c1a2e',borderRadius:8,border:'1px solid '+(isLocked?'#c084fc44':'#1c3352')}}>
        <div style={{fontSize:12,color:'#4d7a9e'}}>{secPicks}/6 picks en Grupo {group}{isLocked&&<span style={{marginLeft:8,color:'#c084fc',fontWeight:700}}>{'\uD83D\uDD12'} BLOQUEADO</span>}</div>
      ...
      {isLocked&&<div style={{...S.cardD,border:'1px solid #c084fc44',textAlign:'center',padding:'16px 14px',marginBottom:12}}>
        <div style={{fontSize:14,fontWeight:700,color:'#c084fc',marginBottom:4}}>{'\uD83D\uDD12'} Grupo {group} Bloqueado</div>
        <div style={{fontSize:12,color:'#4d7a9e'}}>Tus picks estan confirmados. Solo el admin puede desbloquear.</div>
      </div>}
      ...
      {filtered.map(m=>{
        ...
        return(
          <div key={m.id} style={{...S.mc,borderColor:locked?'#c084fc22':pred?'#f0c04033':'#1c3352'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              <span style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,fontWeight:700}}>JORNADA {m.matchday}<span style={{color:'#4d7a9e',fontWeight:800,marginLeft:6}}>3pts</span></span>
...
function BracketTab({pid,participants,predictions,lockedPicks,koResults,onKoPick,onLockKoSection,onBack}){
  ...
  if(!allGroupsLocked) return(
    <div style={S.cardD}>
      <div style={{textAlign:'center',color:'#4d7a9e',padding:'40px 20px',fontSize:14}}>
        Debes bloquear todos los grupos<br/>antes de ver tu bracket.
      </div>
      <button style={{...S.btnOut,width:'100%',marginTop:4}} onClick={onBack}>Volver a Grupos</button>
    </div>
  )

  const roundOrder=[
    {phase:'Ronda de 32',  ids:R32_MATCHES.map(m=>m.id),pts:5,lock:'Ronda de 32',    needLock:''},
    {phase:'Octavos de Final',ids:R16_IDS,pts:5,lock:'Octavos de Final',needLock:'Ronda de 32'},
    {phase:'Cuartos de Final',ids:QF_IDS, pts:5,lock:'Cuartos de Final', needLock:'Octavos de Final'},
    {phase:'Semifinales',  ids:SF_IDS,  pts:7,lock:'Semifinales',      needLock:'Cuartos de Final'},
    {phase:'3er y 4to Puesto',ids:['M103'],pts:7,lock:'Tercer Puesto',  needLock:'Semifinales'},
    {phase:'Gran Final',   ids:['M104'],pts:10,lock:'Gran Final',       needLock:'Semifinales'},
  ]
  ...
  return(
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
        <button style={{...S.btnOut,padding:'6px 12px',fontSize:12}} onClick={onBack}>{'\u2190'} Grupos</button>
        <div style={{flex:1}}>
          <div style={{fontSize:16,fontWeight:700,color:selP?.color||'#f0c040'}}>{selP?.name}</div>
          <div style={{fontSize:11,color:'#4d7a9e'}}>Bracket Eliminatorio Personalizado</div>
        </div>
      </div>

      {roundOrder.map(({phase,ids,pts,lock,needLock})=>{
        const roundLocked=!!lk[lock]
        const prevLocked=!needLock||!!lk[needLock]
        ...
        return(
          <div key={phase}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
              <div style={{flex:1,fontSize:12,letterSpacing:1,color:ptC,fontWeight:700,textTransform:'uppercase'}}>{phase}</div>
              ...
              {roundLocked&&<div style={{fontSize:12,color:'#c084fc'}}>{'\uD83D\uDD12'}</div>}
            </div>
            ...
            {ids.map(mid=>{
              ...
              const isCorrect=picked&&koResult&&picked===koResult
              const isWrong=picked&&koResult&&picked!==koResult
              ...
              return(
                <div key={mid} style={{...S.mc,marginBottom:8,borderColor:roundLocked?'#c084fc22':picked?'#f0c04033':'#1c3352',opacity:(isTBD(m.h)&&isTBD(m.a))?.5:1}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    <span style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,fontWeight:700}}>{mid}</span>
                    {koResult&&<span style={{padding:'2px 9px',borderRadius:4,fontSize:11,fontWeight:700,background:isCorrect?'#1a3a1a':'#3a1a1a',color:isCorrect?'#3ddc84':'#e63946'}}>{isCorrect?'\u2713':'\u2717'} {koResult}</span>}
                    {!koResult&&picked&&<span style={{padding:'2px 8px',borderRadius:4,fontSize:10,fontWeight:700,background:roundLocked?'#c084fc22':'#f0c04022',color:roundLocked?'#c084fc':'#f0c040',border:'1px solid '+(roundLocked?'#c084fc44':'#f0c04044')}}>{roundLocked?'\uD83D\uDD12 ':''}{picked}</span>}
                  </div>
                  ...
                  {!roundLocked&&!isTBD(m.h)&&!isTBD(m.a)&&(
                    <div style={{display:'flex',gap:6}}>
                      {[m.h,m.a].map(team=>(
                        <button key={team} onClick={()=>onKoPick(pid,mid,picked===team?null:team)} style={{flex:1,padding:'9px 8px',borderRadius:7,border:'1px solid',borderColor:picked===team?'#f0c040':'#1c3352',background:picked===team?'#f0c040':'#111d30',color:picked===team?'#050d10':'#4d7a9e',fontFamily:"'Rajdhani',sans-serif",fontSize:12,fontWeight:700,cursor:'pointer',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                          {team}
                        </button>
                      ))}
                    </div>
            ...
            {!roundLocked&&allPicked&&(
              <div style={{padding:'4px 0 16px'}}>
                <button style={{...S.btnLock,width:'100%'}} onClick={()=>setLockModal(lock)}>
                  {'\uD83D\uDD12'} Confirmar y bloquear - {phase}
                </button>
              </div>
            )}
            <div style={{height:1,background:'#1c3352',margin:'12px 0'}}/>
          </div>
        )
      })}
    </div>
...
function AdminTab({participants,results,koResults,predictions,lockedPicks,adminPin,updParticipants,updResults,updKoResults,updLockedPicks,updAdminPin,onReset,adminUnlocked,onNeedPin}){
  ...
  function setKoResult(mid,winner){ const nr={...koResults};if(nr[mid]===winner)delete nr[mid];else nr[mid]=winner;updKoResults(nr) }
  function unlockSec(pid,sk){ const nl={...lockedPicks};if(nl[pid])delete nl[pid][sk];updLockedPicks(nl) }

  function changePin(){
    if(oldPin!==adminPin){setPinMsg({err:true,msg:'El PIN anterior es incorrecto.'});return}
    ...
    setPinMsg({err:false,msg:'PIN cambiado correctamente.'});setTimeout(()=>setPinMsg(null),3000)
  }

  const resFiltered=GROUP_MATCHES.filter(m=>m.phase==='Fase de Grupos'&&m.group===resGroup)
  const doneGrp=resFiltered.filter(m=>results[m.id]).length
  const koIds=KO_ROUND_PHASES[koPhase]||[]
    ...
    <div>
      <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
        {[['participants','\uD83D\uDC65 Jugadores'],['results','\uD83D\uDCCA Fase Grupos'],['ko','\uD83C\uDFC6 Eliminatorias'],['settings','\u2699\uFE0F Config']].map(([id,label])=>(
          <button key={id} onClick={()=>{
            if((id==='results'||id==='ko'||id==='settings')&&!adminUnlocked){onNeedPin(()=>setSec(id));return}
            setSec(id)
      ...
      {sec==='participants'&&<>
        <div style={{...S.cardD,border:'1px solid #f0c04033',marginBottom:12}}>
          <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.7}}>
            {'\u26A0\uFE0F'} <b style={{color:'#f0c040'}}>Solo el admin puede agregar o quitar participantes.</b><br/>
            Necesitas el PIN para cambiar la lista de jugadores.
          </div>
        </div>
        <div style={S.card}>
      ...
      {sec==='results'&&adminUnlocked&&<>
        <div style={{...S.cardD,border:'1px solid #3ddc8433'}}>
          <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.8}}>
            <b style={{color:'#3ddc84'}}>{'\u2705'} Admin activo</b><br/>
            <b style={{color:'#d8eaf7'}}>1</b>=local gana &nbsp;{'\u00B7'}&nbsp; <b style={{color:'#d8eaf7'}}>X</b>=empate &nbsp;{'\u00B7'}&nbsp; <b style={{color:'#d8eaf7'}}>2</b>=visitante gana
          </div>
        </div>
        <GroupPills current={resGroup} onChange={setResGroup}/>
        <div style={{fontSize:11,color:'#4d7a9e',marginBottom:10,textAlign:'right'}}>{doneGrp}/6 registrados en Grupo {resGroup}</div>
        {resFiltered.map(m=>{
      ...
      {sec==='ko'&&adminUnlocked&&<>
        <div style={{...S.cardD,border:'1px solid #3ddc8433'}}>
          <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.7}}>
            <b style={{color:'#3ddc84'}}>{'\u2705'} Admin activo</b><br/>
            Registra que equipo avanza en cada partido eliminatorio.<br/>
            Escribe el nombre exacto como aparece en los brackets.
          </div>
        </div>
        <div style={{display:'flex',gap:6,overflowX:'auto',paddingBottom:8,marginBottom:12,scrollbarWidth:'none'}}>
          {['Ronda de 32','Octavos de Final','Cuartos de Final','Semifinales','Tercer Puesto','Gran Final'].map(ph=>(
        ...
        <div style={{fontSize:11,color:'#4d7a9e',marginBottom:10}}>
          Escribe el nombre del equipo ganador exactamente como aparece en el bracket de cada usuario.
        </div>
        {koIds.map(mid=>{
          ...
          return(
            <div key={mid} style={{...S.mc,borderColor:winner?'#3ddc8433':'#1c3352',marginBottom:8}}>
              ...
              <input style={{...S.input,marginBottom:8}} placeholder={'Equipo ganador de '+mid} value={winner} onChange={e=>{const nr={...koResults};if(e.target.value)nr[mid]=e.target.value;else delete nr[mid];updKoResults(nr)}}/>
            </div>
          )
        })}
      </>}
      ...
      {sec==='settings'&&adminUnlocked&&<>
        <div style={S.card}>
          ...
          <div style={{fontSize:12,color:'#4d7a9e',marginBottom:12,lineHeight:1.6}}>
            Para cambiar el PIN necesitas ingresar el PIN actual primero.<br/>
            <b style={{color:'#f0c040'}}>PIN actual por defecto: {DEFAULT_PIN}</b>
          </div>
          ...
          <div style={{marginBottom:8}}>
            <div style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,marginBottom:4,fontWeight:700}}>NUEVO PIN (min. 4 digitos)</div>
          ...
          <div style={{marginBottom:12}}>
            <div style={{fontSize:10,color:'#4d7a9e',letterSpacing:1,marginBottom:4,fontWeight:700}}>CONFIRMAR NUEVO PIN</div>
            <input style={S.input} type="password" inputMode="numeric" placeholder="Repite el PIN" value={cPin} onChange={e=>setCPin(e.target.value)}/>
          </div>
        ...
        <div style={S.cardD}>
          <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.9}}>
            ...
            - Los picks bloqueados solo el admin puede desbloquear
          </div>
        </div>
      </>}
    </div>
...
export default function App(){
  ...
  const board=useMemo(()=>calcBoard(participants,predictions,results,koResults,lockedPicks),[participants,predictions,results,koResults,lockedPicks])

  useEffect(()=>{
    if(!db){setFatalError('Firebase no inicializado. Verifica el firebaseConfig.');setLoading(false);return}
  ...
  function handlePick(pid,mid,val){
    const lk=lockedPicks[pid]||{}
    // Find which group the match belongs to
    const g=mid[1]
    if(lk['GRP-'+g]) return
    const pp={...(predictions[pid]||{})};if(val===null)delete pp[mid];else pp[mid]=val
    const newPreds={...predictions,[pid]:pp};setPreds(newPreds);save(bun({predictions:newPreds}))
  }
  ...
  function handleKoPick(pid,mid,team){
    ...
    if(lk[phase]) return
    const pp={...(predictions[pid]||{})}
    const kp={...(pp.__k||{})};if(team===null)delete kp[mid];else kp[mid]=team
    pp.__k=kp
    const newPreds={...predictions,[pid]:pp};setPreds(newPreds);save(bun({predictions:newPreds}))
  }
  ...
  // Open bracket for a participant: auto-generate if needed
  function openBracket(pid){
    ...
    if(!pp.__b||!Object.keys(pp.__b).length){
      ...
      const newPreds={...predictions,[pid]:newPp}
      setPreds(newPreds);save(bun({predictions:newPreds}))
    }
    setBracketPid(pid);setTab('bracket')
  }

  // Admin PIN request
  function handleNeedPin(cb){
    setPendingCb(()=>cb);setShowPin(true)
  }
