...
function calcGroupStandings(pid, predictions){
  ...
  GROUPS.forEach(g=>{
    ...
    pairs.forEach(([i,j])=>{
      ...
      if(pick==='1') pts[i]+=3
      else if(pick==='X'){pts[i]+=1;pts[j]+=1}
      else if(pick==='2') pts[j]+=3
    })
    const rank=[0,1,2,3].sort((a,b)=>pts[b]-pts[a]||a-b)
    const t=TEAMS[g]
    standings[g]={
      p1:{name:t[rank[0]][0],flag:t[rank[0]][1],pts:pts[rank[0]]},
      p2:{name:t[rank[1]][0],flag:t[rank[1]][1],pts:pts[rank[1]]},
      p3:{name:t[rank[2]][0],flag:t[rank[2]][1],pts:pts[rank[2]]},
      p4:{name:t[rank[3]][0],flag:t[rank[3]][1],pts:pts[rank[3]]},
    }
  })
  return standings
}
...
function generateR32(pid, predictions){
  ...
  const thirds = GROUPS.map(g=>({group:g,...s[g].p3})).sort((a,b)=>b.pts-a.pts||a.group.localeCompare(b.group))
...
function calcBoard(participants, predictions, results, koResults, lockedPicks){
  return [...participants].map(p=>{
    ...
    GROUP_MATCHES.forEach(m=>{
      if(pp[m.id]&&results[m.id]){played++;if(pp[m.id]===results[m.id]){pts+=3;correct++}}
    })
    // Knockout
