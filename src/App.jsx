...
function officialBracket(results,koResults){
  try{
...
function MatchesTab({results,koResults,predictions,participants}){
  ...
  const offBracket=useMemo(()=>officialBracket(results,koResults),[results,koResults])
  return <div>
...
function BracketTab({pid,participants,predictions,lockedPicks,results,koResults,onKoPick,onLockKoSection,onBack}){
  const[lockModal,setLockModal]=useState(null)
  ...
  // Official bracket — same for all players, auto-updates from admin results
  const bracket=useMemo(()=>officialBracket(results||{},koResults||{}),[results,koResults])
...
function AdminTab({participants,results,koResults,predictions,lockedPicks,adminPin,updParticipants,updResults,updKoResults,updLockedPicks,updAdminPin,onReset,adminUnlocked,onNeedPin}){
  ...
  return <div>
    ...
    {sec==='ko'&&adminUnlocked&&<>
      <div style={{...S.cdk,border:'1px solid #3ddc8433'}}>
        <div style={{fontSize:12,color:'#4d7a9e',lineHeight:1.7}}><b style={{color:'#3ddc84'}}>{'\u2705'} Admin activo</b><br/>Registra el equipo que avanza en cada partido. El bracket de todos los jugadores se actualizara automaticamente.</div>
      </div>
  ...
  useEffect(()=>{
    ...
    try{
      const unsub=onValue(ref(db,DB_PATH),snap=>{
        try{const d=snap.val();if(d){if(d.participants)setP(toArr(d.participants));if(d.results)setR(d.results||{});if(d.koResults)setKoR(d.koResults||{});if(d.predictions)setPreds(d.predictions||{});if(d.lockedPicks)setLocked(d.lockedPicks||{});if(d.adminPin)setAdminPin(d.adminPin)}}catch(e){}
        setLoading(false)
  ...
  return(
    <div style={S.app}>
      ...
      <div style={S.con}>
        ...
        {tab==='bracket'&&bracketPid&&<BracketTab pid={bracketPid} participants={participants} predictions={predictions} lockedPicks={lockedPicks} results={results} koResults={koResults} onKoPick={handleKoPick} onLockKoSection={handleLock} onBack={()=>setTab('picks')}/>}
